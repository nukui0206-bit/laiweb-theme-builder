...
共通トップページ部品のON/OFFは管理画面で選び、表示位置はindex.phpで自由に調整できる
```
...
機能実装をある程度進めた後で、共通パーツのデザインテンプレートをまとめて整える。
...
```text
まずは管理画面から選択できる仕組みを優先して作る。
各テンプレートの見た目、余白、配色、画像の扱い、文言は後半でまとめて調整する。
案件ごとの自由度を残すため、最終的にもPHPテンプレートとadd.cssで調整できる構造を維持する。
```
